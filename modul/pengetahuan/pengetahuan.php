<title>Pengetahuan - Sistem Pakar</title>
<?php
...
switch($_GET[act]){
  ...
  $tampil=mysqli_query($conn,"SELECT * FROM basis_pengetahuan ORDER BY kode_pengetahuan");
	...
	if ($_POST[Go]){
			$numrows = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM basis_pengetahuan b,kerusakan p where b.kode_kerusakan=p.kode_kerusakan AND p.nama_kerusakan like '%$_POST[keyword]%'"));
			if ($numrows > 0){
  ...
  case "tambahpengetahuan":
	echo "	<div class='alert alert-success alert-dismissible'>
				...
				<b>Contoh:</b><br>
				Jika kepercayaan <b>(MB)</b> anda terhadap gejala Komputer Tidak ada tampilan untuk Kerusakan VGA adalah <b>0.8 (Hampir Pasti)</b><br>
				Dan ketidakpercayaan <b>(MD)</b> anda terhadap gejala Komputer tidak ada tampilan untuk kerusakan VGA adalah <b>0.2 (Hampir Mungkin)</b><br><br>
				<b>Maka:</b> CF(Pakar) = MB – MD (0.8 - 0.2) = <b>0.6</b> <br>
				Dimana nilai kepastian anda terhadap gejala Komputer tidak ada tampilan untuk Kerusakan VGA adalah <b>0.6 (Kemungkinan Besar)</b>
              </div>
